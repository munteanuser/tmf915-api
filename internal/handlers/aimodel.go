package handlers

import (
	"context"
	"database/sql"
	"net/http"

	"github.com/tmf915-api/internal/events"
	"github.com/tmf915-api/internal/models"
)

type aiModelRepo interface {
	List(ctx context.Context, offset, limit int) ([]models.AiModel, int, error)
	Get(ctx context.Context, id string) (*models.AiModel, error)
	Create(ctx context.Context, in models.AiModelCreate) (*models.AiModel, error)
	Update(ctx context.Context, id string, in models.AiModelUpdate) (*models.AiModel, error)
	Delete(ctx context.Context, id string) error
}

type AiModelHandler struct {
	repo       aiModelRepo
	dispatcher *events.Dispatcher
}

func NewAiModelHandler(repo aiModelRepo, d *events.Dispatcher) *AiModelHandler {
	return &AiModelHandler{repo: repo, dispatcher: d}
}

func (h *AiModelHandler) List(w http.ResponseWriter, r *http.Request) {
	p := ParseListParams(r)
	items, total, err := h.repo.List(r.Context(), p.Offset, p.Limit)
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "500", "Internal Server Error", err.Error())
		return
	}
	SetPaginationHeaders(w, len(items), total)
	WriteJSON(w, http.StatusOK, items)
}

func (h *AiModelHandler) Create(w http.ResponseWriter, r *http.Request) {
	var in models.AiModelCreate
	if err := DecodeJSON(r, &in); err != nil {
		WriteError(w, http.StatusBadRequest, "400", "Bad Request", err.Error())
		return
	}
	if in.Name == "" {
		WriteError(w, http.StatusBadRequest, "400", "Bad Request", "name is required")
		return
	}
	item, err := h.repo.Create(r.Context(), in)
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "500", "Internal Server Error", err.Error())
		return
	}
	h.dispatcher.Dispatch("AiModelCreateEvent", item)
	WriteJSON(w, http.StatusCreated, item)
}

func (h *AiModelHandler) Get(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	item, err := h.repo.Get(r.Context(), id)
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "500", "Internal Server Error", err.Error())
		return
	}
	if item == nil {
		WriteError(w, http.StatusNotFound, "404", "Not Found", "AiModel not found")
		return
	}
	WriteJSON(w, http.StatusOK, item)
}

func (h *AiModelHandler) Update(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	var in models.AiModelUpdate
	if err := DecodeJSON(r, &in); err != nil {
		WriteError(w, http.StatusBadRequest, "400", "Bad Request", err.Error())
		return
	}
	item, err := h.repo.Update(r.Context(), id, in)
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "500", "Internal Server Error", err.Error())
		return
	}
	if item == nil {
		WriteError(w, http.StatusNotFound, "404", "Not Found", "AiModel not found")
		return
	}
	h.dispatcher.Dispatch("AiModelAttributeValueChangeEvent", item)
	WriteJSON(w, http.StatusOK, item)
}

func (h *AiModelHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	err := h.repo.Delete(r.Context(), id)
	if err == sql.ErrNoRows {
		WriteError(w, http.StatusNotFound, "404", "Not Found", "AiModel not found")
		return
	}
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "500", "Internal Server Error", err.Error())
		return
	}
	h.dispatcher.Dispatch("AiModelDeleteEvent", map[string]string{"id": id})
	w.WriteHeader(http.StatusNoContent)
}
